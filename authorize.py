...
def get_access_token(username, client):
    ...
    server_thread.start()
    time.sleep(1)
    authorize_url = client.authorization_url(client_id=YOUR_CLIENT_ID_HERE, scope="write", redirect_uri='http://127.0.0.1:5000/authorized')
    ...
    while code == b'No Authorized':
        try:
            response = requests.get('http://127.0.0.1:5000/code', timeout=4)
    ...
    access_token = client.exchange_code_for_token(client_id=YOUR_CLIENT_ID_HERE,
                                                  client_secret='YOUR CLIENT SECRET CODE HERE',
                                                  code=code)
